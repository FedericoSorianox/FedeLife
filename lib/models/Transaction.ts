 ...
 */
transactionSchema.methods.getSummary = function(): any {
    return {
        id: this._id,
        type: this.type,
 ...
 */
transactionSchema.methods.isRecent = function(): boolean {
 ...
 */
transactionSchema.methods.isCurrentMonth = function(): boolean {
    const now = new Date();
 ...
 */
transactionSchema.methods.convertToUserCurrency = async function(userBaseCurrency: string) {
    try {
        // Import dinámico del servicio de exchange rate
        const { exchangeRateService } = await import('../../server/services/exchangeRate');
 ...
 */
transactionSchema.methods.getAmountInCurrency = async function(targetCurrency: string): Promise<number> {
    try {
        // Import dinámico del servicio de exchange rate
        const { exchangeRateService } = await import('../../server/services/exchangeRate');
...
const TransactionModel = mongoose.models.Transaction || mongoose.model<Transaction & mongoose.Document>('Transaction', transactionSchema);
